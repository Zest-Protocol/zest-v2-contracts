...
;; -- Core configuration
;; @mainnet: (define-constant UNDERLYING 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.stbtc-token)
(define-constant UNDERLYING .stbtc-token)
...
;; ============================================================================
;; ERRORS (802xxx prefix for vault-stbtc)
;; ============================================================================
...
(define-private (receive-underlying (amount uint) (account principal))
  (begin
    ;; @mainnet: (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.stbtc-token transfer amount account current-contract none))
    (try! (contract-call? .stbtc-token transfer amount account current-contract none))
    (ok true)))

(define-private (send-underlying (amount uint) (account principal))
  (begin
    ;; @mainnet: (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.stbtc-token transfer amount current-contract account none))
    (try! (contract-call? .stbtc-token transfer amount current-contract account none))
    (ok true)))

(define-private (ubalance)
  ;; @mainnet: (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.stbtc-token get-balance current-contract)))
  (unwrap-panic (contract-call? .stbtc-token get-balance current-contract)))
