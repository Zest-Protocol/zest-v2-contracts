(impl-trait 'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.vault-traits.reserve)
(impl-trait 'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.vault-traits.tokenized-vault)
(impl-trait 'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.vault-traits.lending)
(impl-trait 'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.vault-traits.flashloan)
(use-trait flash-callback 'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.vault-traits.flash-callback)

(define-constant UNDERLYING 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.stbtc-token)
(define-constant NAME "Zest stBTC")
(define-constant SYMBOL "zstBTC")
...
(define-constant ERR-FLASHLOAN-UNAUTHORIZED (err u802023))
(define-constant ERR-NO-DEBT (err u802024))
...
(define-private (receive-underlying (amount uint) (account principal))
  (begin
    (try! (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.stbtc-token transfer amount account current-contract none))
    (ok true)))

(define-private (send-underlying (amount uint) (account principal))
  (begin
    (try! (as-contract? ((with-ft 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.stbtc-token "stbtc" amount))
      (try! (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.stbtc-token transfer amount tx-sender account none))
      true))
    (ok true)))

(define-private (ubalance)
  (unwrap-panic (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.stbtc-token get-balance current-contract)))
...
(define-public (system-repay (amount uint))
  (begin
    (try! (check-caller-auth))
    (asserts! (not (get repay (var-get pause-states))) ERR-PAUSED)
    (asserts! (> amount u0) ERR-AMOUNT-ZERO)
